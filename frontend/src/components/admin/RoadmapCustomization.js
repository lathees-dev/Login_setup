...
const RoadmapCustomization = () => {
  ...
  const handleDragEnd = (result) => {
    ...
    const items = Array.from(roadmapNodes);
  ...
  const handleSaveNode = async () => {
    try {
      let updatedNodes;
      if (editingNode) {
        updatedNodes = roadmapNodes.map(node => 
          node.id === editingNode.id ? { ...newNode, id: node.id } : node
        );
      } else {
        const newId = roadmapNodes.length > 0 
          ? Math.max(...roadmapNodes.map(n => parseInt(n.id))) + 1 
          : 1;
        updatedNodes = [...roadmapNodes, { ...newNode, id: newId }];
      }
      
      const success = await updateRoadmap(updatedNodes);
      if (success) {
        handleCloseDialog();
      } else {
  ...
  const handleSaveRoadmap = async () => {
    try {
      const success = await updateRoadmap(roadmapNodes);
      if (success) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Droppable droppableId="roadmap-nodes-list">
          {(provided, snapshot) => (
            <List
